...
namespace F122::Network {
    PacketCarStatusData::Data::Data(const std::array<std::uint8_t, 47>& bytes) :
            ...
            m_maxRPM{util::convert<uint16>({bytes[17], bytes[18]})},
            m_idleRPM{util::convert<uint16>({bytes[19], bytes[20]})},
            m_maxGears{bytes[21]},
            m_drsAllowed{bytes[22]},
            m_drsActivationDistance{util::convert<uint16>({bytes[23], bytes[24]})},
            m_actualTyreCompound{bytes[25]},
            m_visualTyreCompound{bytes[26]},
            m_tyresAgeLaps{bytes[27]},
            m_vehicleFiaFlags{static_cast<int8>(bytes[28])},
