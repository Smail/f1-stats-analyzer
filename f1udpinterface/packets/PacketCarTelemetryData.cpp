...
namespace F122::Network {
    PacketCarTelemetryData::Data::Data(const std::array<std::uint8_t, 60>& bytes) :
            m_speed{util::convert<uint16>({bytes[0], bytes[1]})},
            ...
            m_clutch{bytes[14]},
            m_gear{static_cast<int8>(bytes[15])},
            m_engineRPM{util::convert<uint16>({bytes[16], bytes[17]})},
            m_drs{bytes[18]},
            m_revLightsPercent{bytes[19]},
            m_revLightsBitValue{util::convert<uint16>({bytes[20], bytes[21]})},
            m_brakesTemperature{
                    {
                            util::convert<uint16>({bytes[22], bytes[23]}),
                            util::convert<uint16>({bytes[24], bytes[25]}),
                            util::convert<uint16>({bytes[26], bytes[27]}),
                            util::convert<uint16>({bytes[28], bytes[29]})
                    },
            },
            m_tyresSurfaceTemperature{{bytes[30], bytes[31], bytes[32], bytes[33]}},
            m_tyresInnerTemperature{{bytes[34], bytes[35], bytes[36], bytes[37]}},
            m_engineTemperature{util::convert<uint16>({bytes[38], bytes[39]})},
            m_tyresPressure{
    ...
    PacketCarTelemetryData::PacketCarTelemetryData(const std::array<std::uint8_t, 1347>& bytes) :
            ...
            m_suggestedGear{static_cast<int8>(bytes[22 * 60 + 24 + 2])} {}
